  ...
  switch(estadoActual) {
    case S0_AUTO:
      ...
      if (digitalRead(botonizq) == LOW) {
        currentPose = (currentPose == 0) ? 4 : currentPose - 1;
        showPose();
        delay(300); // Delay anti-rebote simple
        while(digitalRead(botonizq) == LOW); // Espera a soltar el botón
      }
      
      // Botón derecho - siguiente
      if (digitalRead(botonDer) == LOW) {
        currentPose = (currentPose == 4) ? 0 : currentPose + 1;
        showPose();
        delay(300); // Delay anti-rebote simple
        while(digitalRead(botonDer) == LOW); // Espera a soltar el botón
      }
...
void showPose() {
  ...
  // Aquí irían los movimientos de los servos
